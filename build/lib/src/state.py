# ==========================================================================
#  Projektname · src/state.py
#  ----------------------------------------------------
#  Logik für die Entscheidung, ob bei einem neuen Check eine Benachrichtigung
#
...
def _current_platform(result: RouteCheckResult) -> Optional[str]:
    """Aktuelles Gleis aus dem Result extrahieren.

    Aktuell hat RouteCheckResult nur planned_platform — falls später ein
    `actual_platform` ergänzt wird (für echten Gleiswechsel aus Changes-XML),
    hier zentral umstellen.
    """
    return result.planned_platform
...
def decide_notification(
    ...
    *,
    ...
    prev_platform = previous_state.last_reported_platform
    cur_platform = _current_platform(result)
    ...
    # Greift nur, wenn vorher schon ein Gleis berichtet wurde — sonst wäre's ja
    # die Erstmeldung des Gleises, kein "Wechsel".
    if (
        prev_platform is not None
        and cur_platform is not None
        and cur_platform != prev_platform
    ):
        new_state.last_reported_status = status.value
