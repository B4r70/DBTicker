...
#  Geändert am . : 04.05.2026  — --route-Flag, Notifier auf BartoLink umgestellt,
#                                agent_prompt entfernt
# ------------------------------------------------------------------------------------------
...
#
#                  Modi:
#                    (default)             alle aktiven Routen, mit Wochentag-/
#                                          Fenster-Filter (systemd-Timer-Modus)
#                    --route <id>          nur diese Route, OHNE Filter
#                                          (für manuellen Refresh via BartoLink)
# ------------------------------------------------------------------------------------------
...
def process_route(
    ...
    *,
    ...
    # --- Check ausführen ---
    logger.info(
        "[%s] Starte Check (EVA %d, Abfahrt %s, Linie %s, via %s)",
        route_id, from_eva, route["scheduled_departure"], route["line"], via_name,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    Modi:
      (default)        alle aktiven Routen, mit Wochentag-/Fenster-Filter
      --route <id>     nur diese Route, OHNE Filter (manueller Refresh)
    """
    parser = argparse.ArgumentParser(description="DBTicker — DB-Verbindungs-Überwachung")
    parser.add_argument(
        "--route",
        metavar="ROUTE_ID",
        default=None,
        help=(
            "Nur die angegebene Route prüfen, unabhängig vom Check-Fenster. "
...
def main() -> int:
    ...
    if args.route:
        logger.info(
            "DBTicker-Lauf gestartet (Single-Route: %s): %s",
            args.route, now.isoformat(timespec="seconds"),
        )
    else:
    ...
    # --- Modus 1: Single-Route (manueller Refresh via BartoLink) ---
    if args.route:
        matching = [r for r in routes if r.get("id") == args.route]
        if not matching:
            logger.error("Route '%s' nicht in routes.toml gefunden.", args.route)
            return 2

        route = matching[0]

        # Wochentag- und Check-Fenster-Filter werden bewusst NICHT angewandt —
